...
public class AdminUserController {
    ...
    public ApiResponse<List<UserDTO>> pending() {
        List<UserDTO> result = authRepository.findByEnabledFalse()
                ...
                .toList();
        return ApiResponse.ok(result, "Pending users");
    }
    ...
    public ApiResponse<Object> approve(@PathVariable Long id) {
        ...
        if (Boolean.TRUE.equals(user.getEnabled())) {
            return ApiResponse.ok(null, "Already approved");
        }

        user.setEnabled(true);
        authRepository.save(user);

        return ApiResponse.ok(null, "Approved");
    }
    ...
    public ApiResponse<Object> reject(@PathVariable Long id) {
        ...
        if (Boolean.TRUE.equals(user.getEnabled())) {
            throw new ApiException(ErrorCode.BAD_REQUEST, "Cannot reject an approved user");
        }

        authRepository.delete(user);
        return ApiResponse.ok(null, "Rejected and deleted");
    }
}
